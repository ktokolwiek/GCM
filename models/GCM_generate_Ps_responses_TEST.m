function [train, test] = GCM_generate_Ps_responses_TEST()
...
%% .
    ...
    function [trainingData,ll, testData, ll_no_forgetting] = GCM_generative_model(varargin)
        ...
        trainingData(:,3) = (trainingData(:,1)>2.5) * 2 - 1;
        % ideal feedback
...
feedback_amounts = 1; %1- 100%, 2- some taken out
N_per_cell = 1;
N_repeats = 1;
...
sprintf('ps_id,feedback_type,feedback_amount,length,feedback,ideal,model_no_forg,model_forg\n', 1);
%ftest = fopen(fname_test, 'w');
sprintf('ps_id,feedback_type,feedback_amount,length,model_no_forg,model_forg,ideal\n', 1);
        ...
        for ps=1:N_per_cell
            ...
            for rep = 1:N_repeats
                [train,ll,test,ll_no_forget] = GCM_generative_model('training_set', trainingset,...
                    'test_set', testset,'forget_rate',.5);
            end
