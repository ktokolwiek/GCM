...
for gamma = gammas
    for forget_rate = forget_rates
		...
		[trainData, lls(no_repeats), testData] = GCM_model('gamma', gamma, 'forget_rate',...
                        forget_rate, 'choice_parameter', choice_parameter,...
                        'noise_mu', noise_mu, 'noise_sigma', noise_sigma,...
                        'feedType', fType, 'verbose', -1);
		train_lengths(:,no_repeats) = trainData(:,5);
		train_ideal(:,no_repeats) = trainData(:,8);
		train_model(:,no_repeats) = trainData(:,9);
		test_lengths(:,no_repeats) = testData(:,5);
		test_model(:,no_repeats) = testData(:,8);
